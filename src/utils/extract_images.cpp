...
#define MAX_IMAGES_TO_EXTRACT 320
...
//---------------------------------------------------------------------
int get_color_distance(const Vec3b &color1, const Vec3b &color2)
{
...
//---------------------------------------------------------------------
void flood_fill_background(int row, int col, const Mat &picture, char** filled_matrix, int color_tolerance, int image_size, const Vec3b &initial_color)
{
	...
	if (filled_matrix[row][col] == NOT_MARKED_PIXEL) {
		...
		//	|| get_color_distance(initial_color, picture.at<Vec3b>(Point(row, col + 1))) <= color_tolerance
				)
				)
		...
		//	|| get_color_distance(initial_color, picture.at<Vec3b>(Point(row + 1, col))) <= color_tolerance
				)
			)
...
bool remove_background(Mat &input_image, Mat &out_image)
{
	...
	// top side
	int num_pixels_frame = 1;
	for (int col = 0; col < image_width; col++) 
		for (int i = 0; i < 1; i++){
			flood_fill_background(i, col, input_image, matrix, color_distance, image_width, input_image.at<Vec3b>(i, col));
		}
	// left side
	for (int row = 0; row < image_height; row++) 
		for (int i = 0; i < num_pixels_frame; i++) {
			flood_fill_background(row, i, input_image, matrix, color_distance, image_width, input_image.at<Vec3b>(row, i));
		}
	// right side
	for (int row = 0; row < image_height; row++) 
		for (int i = 0; i < num_pixels_frame; i++) {
			flood_fill_background(row, image_width - 1 - i, input_image, matrix, color_distance, image_width, input_image.at<Vec3b>(row, image_width - 1 - i));
		}
...
int main(void)
{
	...
	while (1) {
		...
		//detailEnhance(smaller_image, smaller_image, 50, 0.1);
		/*
		edgePreservingFilter( smaller_image, smaller_image, 1, 
			0, // 0-200 
			0.001 // 0-1
		);
		*/
